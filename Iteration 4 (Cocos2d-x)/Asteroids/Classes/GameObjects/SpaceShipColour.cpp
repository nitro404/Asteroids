...
Colour SpaceShipColours::getColour(const std::string & data) {
	SpaceShipColour colour = parseFrom(data);
...
SpaceShipColours::SpaceShipColour SpaceShipColours::parseFrom(const char * data) {
	...
	std::string colourString = Utilities::trimString(std::string(data));

	for(int i=static_cast<int>(Invalid)+1;i<static_cast<int>(NumberOfColours);i++) {
		if(Utilities::compareStringsIgnoreCase(colourString.data(), colourStrings[i]) == 0) {
...
SpaceShipColours::SpaceShipColour SpaceShipColours::parseFrom(const std::string & data) {
