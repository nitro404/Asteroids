...
Colour::Colour(const unsigned char colour[3], unsigned char alpha)
	: r(colour[0])
	, g(colour[1])
	, b(colour[2])
	, a(alpha) {
...
Colour::Colour(const unsigned char colour[4])
	: r(colour[0])
	, g(colour[1])
	, b(colour[2])
	, a(colour[3]) {
...
void Colour::scale(float factor, bool alpha) {
	...
	r = static_cast<unsigned char>(Math::clamp(scaledRed, 0, 255));
	g = static_cast<unsigned char>(Math::clamp(scaledGreen, 0, 255));
	b = static_cast<unsigned char>(Math::clamp(scaledBlue, 0, 255));
	if(alpha) {
		a = static_cast<unsigned char>(Math::clamp(scaledAlpha, 0, 255));
	}
}
...
Colour Colour::scaled(float factor, bool alpha) const {
	...
	return Colour(static_cast<unsigned char>(scaledRed),
				  static_cast<unsigned char>(scaledGreen),
				  static_cast<unsigned char>(scaledBlue),
		  alpha ? static_cast<unsigned char>(scaledAlpha) : a);
}
...
Colour Colour::lerp(const Colour & c, const Colour & d, float amount) {
	...
	return Colour(static_cast<unsigned char>(lerpedRed),
				  static_cast<unsigned char>(lerpedGreen),
				  static_cast<unsigned char>(lerpedBlue),
				  static_cast<unsigned char>(lerpedAlpha));
}

/*
Colour Colour::parseFrom(const char * data, bool * valid) {
...
#if USE_QT
Colour Colour::parseFrom(const QString & data, bool * valid) {
	static const QRegExp     nonIntegerRegExp("[^-0-9]+");
	static const QRegExp nonIntegerTrimRegExp("(^[^0-9]+)|([^0-9]+$)");
	...
	QStringList parts = QString(data).replace(nonIntegerTrimRegExp, "").split(nonIntegerRegExp, QString::SkipEmptyParts);
	...
	for(int i=0;i<parts.size();i++) {
		value = Utilities::parseInteger(parts[i], &success);
		...
		newColour.c[i] = static_cast<unsigned byte>(value);
	}
...
#if USE_STL
Colour Colour::parseFrom(const std::string & data, bool * valid) {
	...
	for(int i=0;i<static_cast<int>(formattedData.length());i++) {
		if(formattedData[i] == ' ' || i == static_cast<int>(formattedData.length()) - 1) {
			...
			newColour.c[index++] = static_cast<unsigned char>(value);
		}
	}
...
#if USE_QT >= USE_STL
QString Colour::toString() const {
	return QString("%1, %2, %3, %4").arg(static_cast<int>(r)).arg(static_cast<int>(g)).arg(static_cast<int>(b)).arg(static_cast<int>(a));
}
#elif USE_STL
